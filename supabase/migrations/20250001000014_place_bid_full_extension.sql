-- =============================================================
-- Migration 0014: place_bid full extension
-- Each bid now extends the timer by a FULL +5s from its current end,
-- instead of being capped at TIMER_SECONDS-from-now. Previously a bid placed
-- early in the 30s window only nudged the timer a couple of seconds (the
-- LEAST(now()+30s, …) cap); now every bid reliably adds +5 (snipe-protection),
-- so a live bidding war keeps the auction open while people keep bidding.
-- Only step 6 changed vs migration 0006.
-- =============================================================
...
BEGIN
  ...
  -- 6. Compute new timer:
  --    Always add a full BID_EXTENSION seconds onto the current timer end.
  --    No upper cap — every bid reliably extends by +5s.
  v_new_timer := v_state.timer_ends_at + (BID_EXTENSION || ' seconds')::interval;
